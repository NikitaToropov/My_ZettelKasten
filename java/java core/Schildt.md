...
1. чтобы определить общее для неродственных классов
2. и ограничить способ обращения к разнородным реализациям сигнатурой методов интерфейса
...
* если класс реализует 2 ИФ с одинаковыми по сигнатуре дефолтными методами
    * переопределить в классе
        * нет ошибки
    * оставить как есть
        * ошибка
* В случаях с расширения ИФ с одинаковыми по сигнатуре дефолтными методами
...
* все наследуют класс `Throwable`
  * Exception - должна перехватывать прикладная программа. Его наследуют при генерации "своих" исключений
...
- можно пробрасыват уже пойманные экземпляры исключений
  - `throw exception`
...
- обязательно для всех исключений кроме
