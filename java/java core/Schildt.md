...
* Для чего
    * полностью абстрагироваться от реализации
        * реализации доступны большему набору классов
        * интерфейсы изолируют определение методов от иерархии наследования
            * иерархия ИФ не совпадает с иерархией классов
                * классы никак не связанные между собой могут реализовывать один интерфейс
    * обеспечить динамическое разрешение вызовов методов
...
[Как я понял]
чтобы определить общее для неродственных классов и ограничить способ обращения к ним сигнатурой методов интерфейса   
...
!! До JDK8 в интерфейсах не было реализаций
